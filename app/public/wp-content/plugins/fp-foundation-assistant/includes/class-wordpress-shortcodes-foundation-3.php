...
class FP_Tabs { 
	...
	public function fp_tabs( $atts, $content ) { 
		...
		if ( $vertical == 'true' ) { 
			$html .= '<div class="row collapse"><div class="four columns">';
		 } 
		...
		if ( $vertical == 'true' ) { 
			$html .= '<dl class="'.esc_html( $fp_globals[ 'vertical_class' ]  ).' tabs">';
		...
		if ( $vertical == 'true' ) { 
			$html .= '</div><div class="eight columns">';
		 }
	...
	public function fp_tab( $atts, $content = null ){ 
		...
		$fp_globals[ 'tab_id' ]  = 'tab'.$fp_globals[ 'counter_tab_wraps' ] .$fp_globals[ 'counter_tabs' ] ;
		...
		if ( $fp_globals[ 'tab_title' ]  ){ 
			...
			if ( $open == 'true' ) { 			
				...
				$html = "<dd class='".esc_html( $fp_globals[ 'open_tab' ]  ).esc_html( $class )."'><a href=#".esc_html( $fp_globals[ 'tab_id' ]  ).">". esc_html( $title ) ."</a></dd>";
			 } else { 
				if ( !empty( $class ) ) { $class = ' class='.$class; } 
				$html = "<dd".esc_html( $class )."><a href=#".esc_html( $fp_globals[ 'tab_id' ]  ).">". esc_html( $title ) ."</a></dd>";
			 }	

		 } else { 

			if ( $open == 'true' ) { 				
				$html = "<li id='".esc_html( $fp_globals[ 'tab_id' ]  )."Tab' class='".esc_html( $fp_globals[ 'open_tab' ]  )."'><p class='fp-tab-content'>". do_shortcode( $content )."</p></li>";
			 } else { 
				$html = "<li id='".esc_html( $fp_globals[ 'tab_id' ]  )."Tab'><p class='fp-tab-content'>". do_shortcode( $content )."</p></li>";
			 }	
		 }
...
class FP_Accordion { 
	...
	public function fp_accordion_wrap( $atts, $content ) { 
		...
		$html = '<ul class="accordion'.esc_html( $class ).'">';		
		$html .=  do_shortcode( $content );
		$html .= '</ul>';
	...
	public function fp_accordion( $atts, $content = null ){ 
		...
		extract( $atts );
			...
			if ( $open == 'true' ) { 				
				$html = "<li class='".esc_html( $fp_globals[ 'open_tab' ]  )."'>";
			 } else { 
				$html = "<li>";
			 }	

			$html .= "<div class='title'><h5>".esc_html( $title )."</h5></div>";
			$html .= "<div class='content".esc_html( $class )."'>";
			$html .= do_shortcode( $content );
			$html .= "</div>";
			$html .= "</li>";
...
class FP_Menu { 
	...
	public function fp_menu( $atts, $content ) { 
		...
		$html = '<ul class="fp-menu nav-bar'.esc_html( $class ).'">';		
		$html .=  do_shortcode( $content );
		$html .= '</ul>';
	...
	public function fp_menu_item( $atts, $content ){ 
        ...
        $content = str_replace( array( '</p>', '<p>' ), array( '', '' ), $content );
		$atts = shortcode_atts( array( 
				'link' => __( '#', 'fp-foundation-assistant' ), 
		...
		extract( $atts );
				...
				$html = '<li class="has-flyout"><a href="'.esc_url( $link ).'">'.esc_html( $title ).'</a>'. do_shortcode( $content ).'</li>';
    ...
    public function fp_menu_nested( $atts, $content ) { 
		...
		$html = '<a href="#" class="flyout-toggle"><span> </span></a><ul id="'.esc_html( $fp_globals[ 'menu_id' ]  ).'" class="flyout'.esc_html( $class ).'">';		
		$html .=  do_shortcode( $content );
		$html .= '</ul>';
    ...
    public function fp_submenu_item( $atts, $content ){ 
		...
		$html = '<li'.esc_html( $class ).'><a href="'.esc_html( $link ).'">'.esc_html( $title ).'</a>'. do_shortcode( $content ).'</li>';
...
class FP_Button { 
	...
	public function fp_button( $atts, $content ) { 
		...
		if ( in_array( $size, array( 'tiny', 'small', 'medium', 'large' ), true ) ) { $button_classes[] = $size.' '; }
		if ( in_array( $color, array( 'secondary', 'success', 'alert' ), true ) ) { $button_classes[] = $color.' '; } 
...
class FP_Callout { 
	...
	public function fp_callout( $atts, $content ) { 
		...
		if ( in_array( $color, array( 'secondary', 'success', 'alert', 'primary' ), true ) ) { $color = $color.' '; } else { $color = ''; }
		if ( !empty( $class ) ) { $class = $class.' '; }
		
		$html = '<div class="'.esc_html( $color ).esc_html( $class ).'alert-box" data-alert>';
		$html .=  do_shortcode( $content );
...
class FP_Dropdown { 
	...
	public function fp_dropdown( $atts, $content ) { 
		...
		$html = '<ul class="nav-bar fp-dropdown'.esc_html( $class ).'">';
		$html .= '<li class="has-flyout">';
		$html .= '<a href="#">'.esc_html( $title ).'</a>';		
		$html .= '<a href="#" class="flyout-toggle"><span> </span></a><div class="flyout">';			
		$html .=  do_shortcode( $content );
		$html .= '</div>';
		$html .= '</li></ul>';
...
class FP_Interchange { 
	...
	public function fp_interchange( $atts, $content ) { 
		...
		extract( $atts );
			...
			$html .= '<img class="hide-for-small show-for-medium-down" src="'.esc_url( $medium ).'">';	
			$html .= '<img class="show-for-small" src="'.esc_url( $small ).'">';	
			$html .= '<img class="show-for-xlarge" src="'.esc_url( $large ).'">';		
...
class FP_Orbit { 
	...
	public function fp_orbits( $atts, $content ) { 
		...
		$fp_globals[ 'orbit_id' ]  = 'orbit-id-'.uniqid();		
		...
		$html = '<ul id="'.esc_html( $fp_globals[ 'orbit_id' ]  ).'" class="'.esc_html( $class ).'orbit">';
		$html .=  do_shortcode( $content );
		$html .= '</ul>';
...
class FP_Progress { 
	...
	public function fp_progress( $atts, $content ) { 
		...
		if ( in_array( $size, array( 'small', 'large' ), true ) ) { if ( $size == 'small' ) { $progress_classes[] = 'eight '; } else { $progress_classes[] = 'twelve '; } }
		if ( !empty( $class ) ) { $class = $class.' '; }
...
class FP_Reveal { 
	...
	public function fp_reveal( $atts, $content ) { 
		...
		if ( in_array( $size, array( 'expand', 'small', 'medium', 'large', 'xlarge' ), true ) ) { $size = $size.' '; } else { $size = ''; }
		if ( $title == '' ) { $title = __( 'Click here for a modal', 'fp-foundation-assistant' ); } 
		if ( !empty( $class ) ) { $class = $class.' '; }

		$html = '<div class="'.esc_html( $size ).esc_html( $class ).'reveal-modal" id="'.esc_html( $id ).'">';
		$html .=  do_shortcode( $content );
		$html .= '<a class="close-reveal-modal" aria-label="'.__( 'Close', 'fp-foundation-assistant' ).'">&#215;</a></div>';
		$html .= '<p><a class="fp-reveal-link" data-reveal-id="'.esc_html( $id ).'">'.esc_html( $title ).'</a></p>';
...
class FP_Tooltip { 
	...
	public function fp_tooltip( $atts, $content ) { 
		...
		if ( in_array( $shape, array( 'square' ), true ) ) { $tooltip_classes[] = ' noradius'; } 
...
class FP_Visible { 
	...
	public function fp_visible( $atts, $content ) { 
		...
		$options = array( 
			...
			$show_medium_down, $show_medium_only, $hide_medium_down, $hide_medium_only, 		// Medium Visibility Classes
...
class FP_Grid { 
	...
	public function fp_rows( $atts, $content ) { 
		...
		$atts = shortcode_atts( array( 
				...
				// Not Supported in F3; Included to support compatibility between the versions;
				'medium_up' => '', 
				'large_up' => '', 
			 ), $atts
		 );
		...
		$grid_columns = array( 
			...
			array( 'number' => 6, 'text' => 'six' ), 
			array( 'number' => 7, 'text' => 'seven' ), 
			array( 'number' => 8, 'text' => 'eight' ), 
			array( 'number' => 9, 'text' => 'nine' ), 
			array( 'number' => 10, 'text' => 'ten' ), 
			array( 'number' => 11, 'text' => 'eleven' ), 
			array( 'number' => 12, 'text' => 'twelve' )
		 );

		foreach ( $grid_columns as $grid_column ) { 

			if ( !empty( $up ) || !empty( $small_up ) ) { 

				if ( $up == $grid_column[ 'number' ]  ) { $block_class[] = ' '.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }
				if ( $small_up == $grid_column[ 'number' ]  ) { $block_class[] = ' mobile-'.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }
				...
				if ( !empty( $large_up ) ) { 
					...
					if ( $large_up == $grid_column[ 'number' ]  ) { $block_class[] = ' '.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }

				 } else if ( empty( $large_up ) && !empty( $medium_up ) ) { 

					if ( $medium_up == $grid_column[ 'number' ]  ) { $block_class[] = ' '.$grid_column[ 'text' ] .'-up'; } else { $block_class[] = ''; }
				 }
			 }
		 }		

		if ( is_array( $block_class ) ) { $block_class = implode( "", $block_class ); }

		if ( empty( $small_up ) && empty( $medium_up ) && empty( $large_up ) && empty( $up ) ) { 

			$fp_globals[ 'block_grids' ]  = 'false';

			$html = '<div class="row'.esc_html( $class ).esc_html( $bool_class ).'">';
			$html .=  do_shortcode( $content );
			$html .= '</div>';
			...
			$html = '<ul class="block-grid '.esc_html( $class ).esc_html( $block_class ).'">';
			$html .=  do_shortcode( $content );
			$html .= '</ul>';
		 }	
	...
	public function fp_columns( $atts, $content ){ 
		...
		$bool_options = array( 
			array( 'bool' => $centered, 'class' => 'centered ' ), 
			array( 'bool' => $end, 'class' => 'end ' ), 			
		 );

		// Not Supported in F3; Included to support compatibility between the versions;
			if ( ( empty( $centered ) && empty( $small_centered ) && empty( $medium_centered ) )  && ( $large_centered == 'true' ) ) { 
				$bool_options[] = array( 'bool' => $large_centered, 'class' => 'centered ' );
			 } else if ( ( empty( $centered ) && empty( $small_centered ) )  && ( $medium_centered == 'true' ) ) { 
				$bool_options[] = array( 'bool' => $medium_centered, 'class' => 'centered ' );
			 } else if ( empty( $centered ) && ( $small_centered == 'true' ) ) { 
				$bool_options[] = array( 'bool' => $small_centered, 'class' => 'centered ' );
			 }
		...
		foreach ( $grid_columns as $grid_column ) { 
			if ( $column == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			if ( $offset == $grid_column[ 'number' ]  ) { $column_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			if ( $push == $grid_column[ 'number' ]  ) { $column_class[] = 'push-'.$grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			if ( $pull == $grid_column[ 'number' ]  ) { $column_class[] = 'pull-'.$grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }

			// Not Supported in F3; Included to support compatibility between the versions;
			if ( empty( $small ) && empty( $medium ) && empty( $large ) && empty( $column ) ) { 

				if ( empty( $column ) ) { $column = esc_html( absint( 12 ) ); }

			} else if ( empty( $small ) && empty( $medium ) && empty( $column ) ) { 

				if ( $large == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }

			 } elseif ( empty( $small ) && empty( $column ) ) { 

				if ( $medium == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }

			 } elseif ( empty( $column ) ) { 

				if ( $small == $grid_column[ 'number' ]  ) { $column_class[] = $grid_column[ 'text' ] .' '; } else { $column_class[] = ''; }
			 }

			if ( empty( $small_offset ) && empty( $medium_offset ) && empty( $offset ) ) { 

				if ( $large_offset == $grid_column[ 'number' ]  ) { $offset_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $offset_class[] = ''; }

			 } elseif ( empty( $small_offset ) && empty( $offset ) ) { 

				if ( $medium_offset == $grid_column[ 'number' ]  ) { $offset_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $offset_class[] = ''; }

			 } elseif ( empty( $offset ) ) { 

				if ( $small_offset == $grid_column[ 'number' ]  ) { $offset_class[] = 'offset-by-'.$grid_column[ 'text' ] .' '; } else { $offset_class[] = ''; }
			 }
		 }

		if ( is_array( $column_class ) ) { $column_class = implode( "", $column_class ); }
		...
		if ( $fp_globals[ 'block_grids' ]  == 'true' ) { 
			...
			$html .= '</li>';
		 	...
		 	if ( !empty( $class ) ) { $class = $class.' '; }
			$html = '<div class="'.esc_html( $bool_class ).esc_html( $column_class ).esc_html( $class ).'columns">';
			$html .=  do_shortcode( $content );
			$html .= '</div>';
		 }				
